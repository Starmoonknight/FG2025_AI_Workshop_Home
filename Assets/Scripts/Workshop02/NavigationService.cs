...
namespace AI_Workshop02
{
    public class NavigationService : MonoBehaviour
    {
        ...
        {
            int startIndex = _boardManager.CoordToIndex(_boardManager.Width / 2, _boardManager.Height / 2);
            if (_boardManager.TryPickRandomReachableGoal(startIndex, minManhattan, _allowDiagonals, out int goalIndex))
            {
        ...
        public void CancelPath(bool clearVisuals = true)
        {
            ...
            if (clearVisuals && _boardManager != null)
            {
                _boardManager.ResetColorsToBase();
                _boardManager.ClearDebugCostsTouched();
            }
        }
        ...
        private IEnumerator AStarRoutine(int startIndex, int goalIndex, Action<List<int>> onPathFound, bool visualize)  
        {
            ...
            if (!_boardManager.IsValidCell(startIndex) || !_boardManager.IsValidCell(goalIndex)) 
            { 
                IsPathComputing = false;
                onPathFound?.Invoke(null); 
                yield break;
            } 
            if (!_boardManager.GetWalkable(startIndex) || !_boardManager.GetWalkable(goalIndex))
            {
                IsPathComputing = false;
                onPathFound?.Invoke(null);
                yield break;
            ...
            if (visualize)
            {
                _boardManager.ResetColorsToBase();
                _boardManager.ClearDebugCostsTouched();
                _boardManager.BuildVisualReachableFrom(startIndex, _allowDiagonals);
                _boardManager.PaintCell(startIndex, _startColor);
                _boardManager.PaintCell(goalIndex, _goalColor);
            }
            ...
            while (_open.Count > 0)
            {
                ...
                if (visualize)
                {
                    _boardManager.PaintCell(currentIndex, _triedColor);
                    _boardManager.SetDebugCosts(currentIndex, _gCost[currentIndex], _hCost[currentIndex], _fCost[currentIndex]);
                ...
                _boardManager.IndexToXY(currentIndex, out int currentX, out int currentY);
                ...
                foreach (var (dx, dy, stepCost) in Neighbors8)
                {
                    ...
                    if (!_boardManager.TryCoordToIndex(newX, newY, out int newIndex)) continue; 
                    if (!_boardManager.GetWalkable(newIndex)) continue;

                    // Disallow diagonal ONLY when BOTH orthogonal side cells are blocked.
                    if (dx != 0 && dy != 0)
                    {
                        bool sideAOpen = _boardManager.TryCoordToIndex(currentX + dx, currentY, out int sideA) && _boardManager.GetWalkable(sideA);
                        bool sideBOpen = _boardManager.TryCoordToIndex(currentX, currentY + dy, out int sideB) && _boardManager.GetWalkable(sideB);
                    ...
                    int terrain = _boardManager.GetTerrainCost(newIndex);
                    ...
                    {                        
                        ...
                        if (_state[newIndex] != 1)
                        {
                            ...
                            if (visualize)
                            {
                                _boardManager.PaintCell(newIndex, _frontierColor);
                                _boardManager.SetDebugCosts(newIndex, _gCost[newIndex], _hCost[newIndex], _fCost[newIndex]);
                            }
                        }
                        else
                        {
            ...
            if (visualize)
            {
                ...
                {
                    _boardManager.PaintCell(path[i], _pathColor);
                    _boardManager.SetDebugCosts(path[i], _gCost[path[i]], _hCost[path[i]], _fCost[path[i]]);
                    if (_stepDelay > 0f) yield return new WaitForSeconds(_stepDelay);
                }
            }
        }
        ...
        {
            _boardManager.IndexToXY(fromIndex, out int fromX, out int fromY);
            _boardManager.IndexToXY(toIndex, out int toX, out int toY);

            int distanceX = Math.Abs(fromX - toX);
            int distanceY = Math.Abs(fromY - toY);

            int minTerrain      = _boardManager.MinTerrainCost; 
